...
:
    ...
    sgm1_("sgm", dimensionSet(-1, -3, 3, 0, 0, 2, 0), nuModel1_->viscosityProperties()), // ADD
    sgm2_("sgm", dimensionSet(-1, -3, 3, 0, 0, 2, 0), nuModel2_->viscosityProperties())  // ADD
{}
...
bool Foam::immiscibleIncompressibleTwoPhaseEHDMixture::readadd() // ADD
{
    if (regIOobject::read())
    {
        ...
        )
        {
            ...
            nuModel1_->viscosityProperties().lookup("sgm") >> sgm1_;
            nuModel2_->viscosityProperties().lookup("sgm") >> sgm2_;
